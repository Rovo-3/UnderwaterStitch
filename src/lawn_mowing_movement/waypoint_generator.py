...
class SweepWPGenerator:
    def __init__(self, init_pos=(0,0), length=0, angle=0, gap=0, iteration=1):
        # (x,y)
    ...
    def generate(self,turn_orientation=1):
        ...
        for _ in range(self.iteration):
            ...
            angle_in_rad = self.angle
            x_add=self.length*math.sin(angle_in_rad)
            y_add=self.length*math.cos(angle_in_rad)
            point1 = (x1+x_add, y1+y_add)
            self.wp.append(point1)
            # add gap 
            angle_in_rad += turn_orientation*(90*(math.pi/180))
            x_add=self.gap*math.sin(angle_in_rad)
            y_add=self.gap*math.cos(angle_in_rad)
